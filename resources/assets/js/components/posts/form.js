...
Vue.component('posts-form', {
    ...
    data() {
        return {
            ...
            tag: '',
            tags: [],
            universities:[ 'جامعة بيرزيت', 'جامعة النجاح','جامعة البوليتكنك', 'جامعة الخليل',
                'الكلية العصرية','جامعة القدس/ أبو ديس',  'دار المعلمين','جامعة فلسطين الأهلية', 'جامعة خضوري',
                'جامعة الأمريكية','جامعات عام'],
            ...
            redirectPath: `${BASE_URL}/dashboard/posts`,
            saveAction: {
                link: `${BASE_URL}/dashboard/posts`,
                type: 'post'
            },
            pickerOptions: {
                disabledDate(date) {
    ...
    mounted() {
        if (this.post_edit) {
            ...
            this.saveAction = {
                link: `${BASE_URL}/dashboard/posts/${this.post_edit.id}`,
                type: 'put'
            };

            axios.get(`${BASE_URL}/dashboard/posts/get_post/` +this.post_edit.id).then(response => {
                ...
                if (this.post) {
                    ...
                    this.university_id = this.post.university_id;
    ...
    methods: {
        search_post(val){
            if(val.length>2){
                axios.post(`${BASE_URL}/dashboard/posts/search`, {
                    key: val
                }).then(response => {
        ...
        saveReturn(){
            this.redirectPath=`${BASE_URL}/dashboard/posts`;
            this.save();
        },
        save() {
              let data = {
        ...
        saveDraft() {
            swal({
                ...
                () => {
                    ...
                    this.saveForm(data);
                    // setTimeout(() => {
                    //         location.href = `${BASE_URL}/dashboard/posts`;
                    // }, 2000)
